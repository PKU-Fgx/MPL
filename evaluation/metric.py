...
class Metric:
    """ Tuple Metric """
    def __init__(self, verbose=False, match_mode='set'):
        self.tp = 0.
        self.gold_num = 0.
        self.pred_num = 0.
...
class RelClassScorer:

    def __init__(self):
        self.ignore_o = True
        self.ace05_label_dict = json.load(open('/shd/fanggx/Paper/CodeCoT/data/train/train/RC/ACE05/label_exp_new.json'))[0]
        self.tacred_label_dict = json.load(open('/shd/fanggx/Paper/CodeCoT/data/train/train/RC/TACRED/label_exp_new.json'))[0]
        self.scierc_label_dict = json.load(open('/shd/fanggx/Paper/CodeCoT/data/train/ood/Formatted/RC_SciERC/label_exp_new.json'))[0]

    @staticmethod
    def get_f1(key, prediction, ignore_o):
        if ignore_o:
            ...
            correct_by_relation = (key == prediction).astype(np.int32).sum()
            guessed_by_relation = len(prediction)
            gold_by_relation = len(key)
    ...
    def change_to_id(self, t, name):
        if name == "tacred":
            id2label = list(self.tacred_label_dict.keys())
        elif name == "ace05":
            id2label = list(self.ace05_label_dict.keys())
        elif name == "scierc":
            id2label = list(self.scierc_label_dict.keys())
        else:
            raise ValueError("?!")
            
        label2id = { l: i for i, l in enumerate(id2label)}

        if t in label2id.keys():
            return label2id[t]
        else:
            return 0

    def load_gold_list(self, label_list):
        name_determine = list()
        for l in label_list[:100]:
            if l in self.tacred_label_dict.keys():
                name_determine.append("tacred")
            elif l in self.ace05_label_dict.keys():
                name_determine.append("ace05")
            else:
                name_determine.append("scierc")
                
        name_determine = sorted(Counter(name_determine).items(), key=lambda x: x[-1], reverse=True)
        name = name_determine[0][0]
        
        self.ignore_o = True if name == "tacred" else False
        
        return [self.change_to_id(l, name) for l in label_list]

    def eval_instance_list(self, gold_instance_list, pred_instance_list):
        output = self.get_f1(np.array(gold_instance_list), np.array(pred_instance_list), self.ignore_o)
        return {"string-rel-strict-F1": output[-1]*100}